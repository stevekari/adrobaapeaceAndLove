...
export default function AdminAuthPage({ onLoginSuccess, onSwitchToMember, showToast }) {
  ...
  // 2. Handle Admin Registration (Single founder initial or subsequent with Admin Code)
  const handleAdminRegister = async (e) => {
    e.preventDefault();
    setAdminError('');

    if (hasAdminAccount && !adminCode.trim()) {
      setAdminError('An administrator account already exists. Please enter your Admin Registration Code provided by the primary administrator.');
      return;
    }
    ...
    try {
      const authData = await api.registerAdmin({
        ...
        adminPasskey: hasAdminAccount ? adminCode.trim().toUpperCase() : null,
      });
          ...
          {activeTab === 'register' && (
            <div className="auth-tab-pane animate-fade-in">
              <div className="auth-card-title-box">
                <div className="admin-badge-pill">
                  <ShieldCheck size={14} />
                  <span>{hasAdminAccount ? 'Additional Administrator Registration' : 'Executive Administration & Company Setup'}</span>
                </div>
                <h2>{hasAdminAccount ? 'Register Additional Administrator' : 'Register Company & Administrator'}</h2>
                <p>
                  {hasAdminAccount 
                    ? 'Register as an executive administrator using your authorized single-use registration code issued by the primary administrator.' 
                    : 'Register your company or association. The registering person is automatically granted default Administrator (ADMIN) privileges.'}
                </p>
              </div>
              ...
              <form onSubmit={handleAdminRegister} className="auth-form">
                {hasAdminAccount && (
                  <div className="auth-input-group animate-fade-in" style={{ backgroundColor: 'rgba(99, 102, 241, 0.08)', padding: '14px', borderRadius: '10px', border: '1px solid rgba(99, 102, 241, 0.25)', marginBottom: '16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px', color: 'var(--primary-color, #4f46e5)', fontWeight: 600, fontSize: '0.88rem' }}>
                      <KeyRound size={16} />
                      <span>Admin Registration Code Required</span>
                    </div>
                    <label style={{ fontSize: '0.82rem', marginBottom: '6px' }}>
                      Authorization Code (Issued by Primary Admin) <span className="req">*</span>
                    </label>
                    <div className="auth-input-wrapper">
                      <KeyRound size={17} className="input-icon" />
                      <input 
                        type="text"
                        placeholder="e.g. ADM-2026-8812 or ASSOC-4412"
                        value={adminCode}
                    ...
                    <span className="input-hint" style={{ fontSize: '0.78rem', marginTop: '6px', color: 'var(--text-muted)' }}>
                      To prevent unauthorized registrations, additional administrators must provide a single-use code generated by the executive admin.
                    </span>
                  </div>
                )}
